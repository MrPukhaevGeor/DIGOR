...
class _WordPageState extends ConsumerState<WordPage>
  ...
  final StreamController<bool> updateStreamController =
      StreamController<bool>();

  Widget interactive({required bool canScroll, required Widget child}) {
    if (false) return SingleChildScrollView(physics: null, child: child);
  ...
  @override
  void dispose() {
    ZoomOverlay.hide(immediately: true);
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final wordProvider = ref.watch(fetchWordProvider(widget.id));
    return wordProvider.when(
                          ...
                          builder: (
                            ...
                            child: Container(
                              ...
                              constraints: BoxConstraints(
                                  minHeight:
                                      MediaQuery.sizeOf(context).height - 195,
      ...
      error: (error, stackTrace) => Scaffold(
        appBar: widget.needAppBar ? AppBar(title: Text(tr('Error'))) : null,
        body: error is DioError && error.error is SocketException
            ? SocketExceptionWidget(
                () => ref.refresh(fetchWordProvider(widget.id)), true)
            : Center(
                child: Padding(
      ...
      loading: () => Scaffold(
        appBar:
            widget.needAppBar ? AppBar(title: Text(tr('translation'))) : null,
